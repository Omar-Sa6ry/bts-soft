...
import { INotificationChannel } from './interfaces/INotificationChannel.interface';
...
export class TelegramChannel implements INotificationChannel {
  ...
  public async send(message: NotificationMessage): Promise<void> {
    ...
    try {
      ...
      console.error(`Failed to send Telegram message to ${recipientId}:`, error);
      throw new Error(`Telegram send error: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}